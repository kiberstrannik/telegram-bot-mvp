...
📖 Технический стиль:
• Не пересказывай прошлое и не повторяй сцены.  
...
Если сцена подразумевает близость — пиши откровенно, чувственно и атмосферно.  

⚠️ Не повторяй предложения, сцены и описания из предыдущих сообщений. Всегда продолжай историю с нового момента.
`.trim();
...
export async function generateSpicyReply(
  ...
  const shortHistory = clipped.slice(-6); // ✅ ограничиваем контекст
  ...
  const directive: Msg = {
    ...
    content: isContinue
      ? "Продолжи сцену естественно, основываясь на предыдущих событиях. " +
        "⚠️ Не повторяй текст, который уже был написан ранее. " +
        "Добавь новое развитие, эмоции, ощущения или реплику. " +
        "Каждое новое сообщение должно продвигать сюжет вперёд. Пиши 1–2 абзаца."
      : "Продолжи историю, реагируя на действия игрока. " +
        "⚠️ Не пересказывай то, что уже произошло. Опиши развитие, новое действие, реакцию или событие. " +
        "Пиши не более 1–2 абзацев.",
  };

  const messages: Msg[] = [
    { role: "system", content: SYSTEM_PROMPT },
    ...(charProfileText ? [{ role: "system" as const, content: charProfileText }] : []),
    ...shortHistory, // ✅ контекст ограничен
    directive,
  ];
